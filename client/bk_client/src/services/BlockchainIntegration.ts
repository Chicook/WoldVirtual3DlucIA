...
class BlockchainIntegration {
  ...
  generateNFTMetadata(
    ...
    const attributes = [
      {
        trait_type: 'Creator',
        value: _sceneMetadata.creator
      },
      {
        trait_type: 'Object Count',
        value: _sceneMetadata.objects.length
      },
      {
        trait_type: 'Creation Date',
        value: new Date(_sceneMetadata.createdAt).toISOString()
      },
      {
        trait_type: 'Version',
        value: _sceneMetadata.version
      }
    ];

    // Añadir atributos de objetos únicos
    const objectTypes = new Set(_sceneMetadata.objects.map((obj: any) => obj.type));
    objectTypes.forEach(type => {
      const count = _sceneMetadata.objects.filter((obj: any) => obj.type === type).length;
      attributes.push({
    ...
    return {
      id: _sceneMetadata.id,
      name: _sceneMetadata.name,
      description: _sceneMetadata.description,
      image: imageUrl,
      attributes,
      properties: {
        files: [
   ...
   */
  async checkTokenBalance(): Promise<string> {
    if (!this.isWalletConnected()) {
      throw new Error('Wallet no conectada');
    }
