...
class SceneLoader {
   ...
   */
  clearAllScenes(): void {
    this.loadedScenes.forEach((_, id) => {
      this.removeScene(id);
    });
    this.currentScene = null;
  }
...
/**
 * Escanea el directorio de mapas y devuelve una lista de archivos de escena disponibles
 * @param {string} mapsDir - Ruta al directorio de mapas (por defecto 'public/maps/')
 * @returns {string[]} Lista de archivos de escena
 */
export function scanAvailableScenes(mapsDir = path.resolve(__dirname, '../../../public/maps/')) {
  if (!fs.existsSync(mapsDir)) return [];
  return fs.readdirSync(mapsDir).filter(file => file.endsWith('.glb') || file.endsWith('.json'));
} 