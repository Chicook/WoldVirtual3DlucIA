...
// ============================================================================
// SISTEMA AVANZADO DE MONITOREO Y MÉTRICAS
// ============================================================================
...
class SchedulerMetrics {
    ...
    recordExecution(taskId, duration, success) {
    ...
    getMetrics() {
        return {
            ...
            successRate: this.metrics.tasksExecuted > 0 ? 
                ((this.metrics.tasksExecuted - this.metrics.tasksFailed) / this.metrics.tasksExecuted) * 100 : 0
        };
    }
    ...
    toCSV(data) {
        const headers = ['timestamp', 'taskId', 'duration', 'success', 'memoryUsage', 'cpuUsage'];
        const rows = data.history.map(h => [
            h.timestamp.toISOString(),
            h.taskId,
            h.duration,
            h.success,
            h.memoryUsage.heapUsed,
            h.cpuUsage.user
        ]);
...
// Extender MetaversoScheduler con métricas
MetaversoScheduler.prototype.metrics = new SchedulerMetrics();

// Sobrescribir executeTask para incluir métricas
const originalExecuteTask = MetaversoScheduler.prototype.executeTask;
MetaversoScheduler.prototype.executeTask = function(task) {
    const startTime = Date.now();
    let success = false;
    
    try {
        originalExecuteTask.call(this, task);
        success = true;
    } catch (error) {
        this.log(`Error ejecutando tarea ${task.id}: ${error.message}`, 'ERROR');
        success = false;
    } finally {
        const duration = Date.now() - startTime;
        this.metrics.recordExecution(task.id, duration, success);
    }
};

// Añadir métodos de métricas al scheduler
MetaversoScheduler.prototype.getMetrics = function() {
    return this.metrics.getMetrics();
};

MetaversoScheduler.prototype.getExecutionHistory = function(limit) {
    return this.metrics.getExecutionHistory(limit);
};

MetaversoScheduler.prototype.exportMetrics = function(format) {
