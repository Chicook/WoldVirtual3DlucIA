...
class CollisionSystem {
   ...
   */
  checkPointCollision(point: THREE.Vector3, radius: number = 0.5): CollisionResult {
   ...
   */
  isValidPosition(position: THREE.Vector3, radius: number = 0.5): boolean {
    const collision = this.checkPointCollision(position, radius);
  ...
  getValidPosition(
    ...
    if (this.isValidPosition(desiredPosition, radius)) {
    ...
    for (let i = 0; i < maxAttempts; i++) {
      ...
      if (this.isValidPosition(testPosition, radius)) {
        return testPosition;
      }
    }
