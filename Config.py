...
class SecurityConfigManager:
            ...
            if lang == 'python':
                if file_path.name == 'requirements.txt':
                    ...
                    try:
                        ...
                        self.log_operation("DEPENDENCY_SKIPPED", f"{lang}: pip no disponible - {file_path}")
                        return False
                    ...
                    try:
                        ...
                        self.log_operation("DEPENDENCY_SKIPPED", f"{lang}: setup.py falló - {file_path}")
                        return False
                ...
                if file_path.name == 'package.json':
                    ...
                    try:
                        ...
                        # Intentar con yarn si npm no está disponible
                        try:
                            subprocess.run(['yarn', '--version'], 
                                         check=True, capture_output=True)
                            subprocess.run(['yarn', 'install'], 
                                         cwd=file_path.parent, check=True, capture_output=True)
                            self.log_operation("DEPENDENCY_INSTALLED", f"{lang}: {file_path} (yarn)")
                            return True
                        except (subprocess.CalledProcessError, FileNotFoundError):
                            self.log_operation("DEPENDENCY_SKIPPED", f"{lang}: npm/yarn no disponible - {file_path}")
                            return False
                ...
                if file_path.name == 'go.mod':
                    try:
                        ...
                        self.log_operation("DEPENDENCY_SKIPPED", f"{lang}: go no disponible - {file_path}")
                        return False
                ...
                if file_path.name == 'Cargo.toml':
                    try:
                        ...
                        self.log_operation("DEPENDENCY_SKIPPED", f"{lang}: cargo no disponible - {file_path}")
                        return False
            
            # Para otros lenguajes, solo registrar que se encontró el archivo
            self.log_operation("DEPENDENCY_FOUND", f"{lang}: {file_path} (no instalado)")
            return True
    ...
    def backup_complete_folder(self, folder_path: Path) -> bool:
        """Hace backup completo de una carpeta antes de limpiarla"""
        try:
            if not folder_path.exists():
                return True
            
            backup_path = self.backup_dir / f"complete_{folder_path.name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            
            # Crear backup completo de la carpeta
            shutil.copytree(folder_path, backup_path, dirs_exist_ok=True)
            
            self.log_operation("COMPLETE_FOLDER_BACKUP", f"{folder_path} -> {backup_path}")
            return True
            
        except Exception as e:
            self.log_operation("COMPLETE_BACKUP_ERROR", f"{folder_path}: {e}")
            return False
    
    def restore_complete_folder(self, folder_name: str) -> bool:
        """Restaura una carpeta completa desde su backup"""
        try:
            # Buscar el backup más reciente de esta carpeta
            backup_pattern = f"complete_{folder_name}_*"
            backup_folders = list(self.backup_dir.glob(backup_pattern))
            
            if not backup_folders:
                self.log_operation("RESTORE_ERROR", f"No se encontró backup para {folder_name}")
                return False
            ...
            # Remover carpeta actual si existe
            ...
            self.log_operation("COMPLETE_FOLDER_RESTORED", f"{folder_name} <- {latest_backup}")
            return True
            
        except Exception as e:
            self.log_operation("RESTORE_ERROR", f"{folder_name}: {e}")
            return False
    ...
    def command_instalar(self):
        ...
        for lang, files in dependency_files.items():
            ...
            for file_path in files:
                ...
                if self.install_dependencies(lang, file_path):
                    ...
                    # Verificar si fue un error o simplemente no disponible
                    if "no disponible" in str(file_path):
                        skipped_count += 1
                    else:
                        error_count += 1
        ...
        print(f"   ⚠️ Herramientas no disponibles: {skipped_count}")
